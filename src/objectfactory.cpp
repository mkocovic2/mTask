...
ObjectFactory::ObjectFactory() {
  commandParse["board"] = [this](std::string command,
                                 std::string name) -> IWorkspace * {
    return this->defineBoard(name);
  };
  commandParse["task"] = [this](std::string command,
                                std::string name) -> IWorkspace * {
    return this->defineTask(name);
  };
}

void ObjectFactory::handleAction(std::string command, std::string type,
